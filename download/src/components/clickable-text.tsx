    ...
    React.useEffect(() => {
        const handleTranslate = async (wordToTranslate: string) => {
            ...
            try {
                ...
                if (!cleanedWord) {
                    setTranslation({ translation: '', explanation: '', synonyms: [], error: 'No translatable word selected.' });
                    return;
                }
                ...
                if (result.translation) {
                    ...
                    const errorResult = { translation: '', explanation: '', synonyms: [], error: result.error || "Unknown error." };
                    setTranslation(errorResult);
                    setCache(prev => ({ ...prev, [wordToTranslate]: errorResult }));
                }
            } catch (error) {
                console.error("Translation failed", error);
                const errorResult = { translation: '', explanation: '', synonyms: [], error: 'Failed to fetch translation.' };
                setTranslation(errorResult);
                setCache(prev => ({ ...prev, [wordToTranslate]: errorResult }));
            } finally {
                setIsLoading(false);
                ...
                <div className="space-y-4 animate-in fade-in duration-300">
                    ...
                    {translation.synonyms && translation.synonyms.length > 0 && (
                        <div className="flex flex-wrap gap-2">
                            <span className="text-xs font-semibold text-muted-foreground mt-1">Synonyms:</span>
